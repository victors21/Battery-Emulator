...
enum bms_charge_discharge_status_enum { IDLE = 0, DISCHARGING = 1, CHARGING = 2 };

static void handle_static_data_modbus_byd();
static void verify_temperature_modbus();
static void handle_update_data_modbusp201_byd();
static void handle_update_data_modbusp301_byd();
static void update_modbus_registers_byd();
...
static void handle_update_data_modbusp201_byd() {
  ...
  system_data[3] = MB_MAX_POWER;  // Id.: p204 Value.: 32000 Scaled value.: 32kWh Comment.: Nominal capacity
  system_data[4] =
      MB_MAX_POWER;  // Id.: p205 Value.: 14500 Scaled value.: 30,42kW  Comment.: Max Charge/Discharge Power=10.24kW (lowest value of 204 and 205 will be enforced by Gen24)
  system_data[5] =
...
static void handle_update_data_modbusp301_byd() {
  ...
  bms_charge_discharge_status_enum bms_char_dis_status;

  if (system_battery_current_dA == 0) {
    bms_char_dis_status = bms_charge_discharge_status_enum::IDLE;
  } else if (system_battery_current_dA < 0) {  //Negative value = Discharging
    bms_char_dis_status = bms_charge_discharge_status_enum::DISCHARGING;
  } else {  //Positive value = Charging
    bms_char_dis_status = bms_charge_discharge_status_enum::CHARGING;
  }
